...
const generateConfetti = ({ corners, ticks, zIndex, particleCount, emojis = [], svgs = [] }) => {
  ...
  const svgShapes = svgs.map((cfg) => {
      ...
      return {
        shape: {
          ...
          render: (ctx, particle) => {
            ...
            ctx.rotate((particle.tilt * Math.PI) / 180);
            const size = 15 * particle.scalar;
            ctx.drawImage(cfg.imageObj, -size / 2, -size / 2, size, size);
            ctx.restore();
          },
        },
        scalar: cfg.scalar || 5,
        isImage: true, // Explicitly set
      };
    }
    ...
    return {
      ...
      color: cfg.color,
      scalar: cfg.scalar || 5,
      isImage: false, // Explicitly set to false for paths
    };
  });
  ...
  const fire = (xOrigin, yOrigin, angle) => {
    ...
    // SVG burst (handles both paths and gradient images)
    svgShapes.forEach((item) => {
      confetti({
        ...base,
        shapes: [item.shape], // Must be an array
        // Only pass colors if it's NOT an image (images use their own internal colors)
        colors: item.isImage ? [] : [item.color],
        particleCount: 12,
        scalar: item.scalar,
        startVelocity: dynamicVelocity,
        gravity: 0.7,
...
const netflixLogoSvgString = `
<svg viewBox="0 0 551.11109 999.99998" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <linearGradient id="g1">
      <stop style="stop-color:#b1060f;stop-opacity:1;" offset="0" />
      <stop style="stop-color:#7b010c;stop-opacity:1" offset="0.62" />
      <stop style="stop-color:#b1060f;stop-opacity:0;" offset="1" />
    </linearGradient>
    <linearGradient id="g2">
      <stop style="stop-color:#b1060f;stop-opacity:1" offset="0" />
      <stop style="stop-color:#7b010c;stop-opacity:1" offset="0.54" />
      <stop style="stop-color:#e50914;stop-opacity:0;" offset="1" />
    </linearGradient>
    <linearGradient xlink:href="#g2" id="gradA" x1="78" y1="423" x2="221" y2="365" gradientUnits="userSpaceOnUse" />
    <linearGradient xlink:href="#g1" id="gradB" x1="456" y1="521" x2="309" y2="583" gradientUnits="userSpaceOnUse" />
  </defs>
  <path fill="url(#gradA)" d="M -1.15 -1.15 2.3 1002.67 C 75.57 988.55 133.19 990.1 198.22 984.23 V 0 Z" />
  <path fill="url(#gradB)" d="m 353.81 0 h 199.38 l 2.3 1000.36 -202.83 -33.42 z" />
  <path fill="#e50914" d="M 1.15 0 C 5.76 11.52 346.9 981.92 346.9 981.92 c 56.05 -0.4 131.21 8.75 205.14 17.28 L 197.07 0 Z" />
</svg>`;

const netflixImg = new Image();
// unescape(encodeURIComponent()) handles special characters better than btoa alone for complex SVGs
netflixImg.src = "data:image/svg+xml;base64," + btoa(unescape(encodeURIComponent(netflixLogoSvgString)));
