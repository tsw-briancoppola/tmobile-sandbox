...
function g(t, e) {
  u()
    .then((i) => {
      ...
      t.data = a;
      e([...t.data], t.currentPage, t.pageLimit);
      t.updatePageNumber(1); // trigger pagination re-render after real data loads
    })
    .catch((i) => {
...
function m() {
  return {
    ...
    registerEventListeners() {
      ...
      this.paginationDigits.forEach((digit) => {
        digit.addEventListener("click", this.setPage.bind(this));
      });
    },
    setPage(t) {
      t.preventDefault();
      const page = parseInt(t.target.textContent);
      if (!isNaN(page)) {
        this.model.updatePageNumber(page);
    ...
    setPaginationView(t) {
      ...
      if (r <= c) {
        ...
        this.paginationDigits.forEach((h) => {
          if (parseInt(h.textContent) === n) h.classList.add("active");
        });
      }
      ...
      if (this.model.currentPage === 1 && r == 0) {
    ...
    setPaginationNavigation() {
      ...
      this.model.filteredData === void 0 || !this.model.filteredData.length ? this.model.data : this.model.filteredData;
    },
    clearActiveStatus() {
  ...
  return {
    ...
    handleSearchInput(t) {
      ...
      if (t.target.value.length > 0) {
        this.setSearchActive(e);
        this.updateSearchTerm(t.target.value);
      }
      if (t.target.value.length === 0 && e.classList.contains("active")) {
        this.resetSearch(e);
    ...
    searchData() {
      ...
      if (!t.length) {
        this.model.noSearchResult("No search result found.");
        this.updatePaginationView(1);
        return;
      }
      this.model.filteredData = [...t];
      this.model.updatePageNumber(1);
      this.updatePaginationView();
    },
...
function w(t) {
  return {
    ...
    updatePageNumber(e) {
      this.currentPage < e ? (this.direction = "up") : (this.direction = "down");
      this.currentPage = e;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function a(s) {
    ...
    p();
    l(s, n, c);
    // Re-render pagination digits now that we have the full model state
    if (t.model) {
      t.setPaginationView();
    }
  }
